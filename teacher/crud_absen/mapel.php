                        ...
                        echo "</br></br>";
 						...
 						while($user_data = mysqli_fetch_array($mapel)) {   
							 ...
							 echo"<h4  >$MAPEL</h4>
							 	...
							 	$result = mysqli_query($mysqli, "SELECT ID_KELAS FROM `belajar` WHERE ID_MAPEL = '$ID_MAPEL'");
								
							 	while($id_kelas = mysqli_fetch_array($result)){
											$kelas= mysqli_query($mysqli, "SELECT * FROM `kelas` WHERE ID_KELAS = '$id_kelas[ID_KELAS]'");
											$KELAS= mysqli_fetch_array($kelas);
											echo "<tr>";
							 				echo "<td class='col-5'>".$KELAS['KELAS']."</td>";
											if($CRUD == 1){
